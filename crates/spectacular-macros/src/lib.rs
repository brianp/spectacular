...
pub fn test_suite(attr_input: TokenStream, item: TokenStream) -> TokenStream {
    let has_suite = if attr_input.is_empty() {
        false
    } else {
        let ident = syn::parse_macro_input!(attr_input as Ident);
        if ident != "suite" {
    ...
    match attr::expand(input, has_suite) {
        Ok(tokens) => tokens.into(),
        Err(e) => e.to_compile_error().into(),
    }
}

/// Marker for test cases in [`test_suite`]. See [`spectacular::test_case`](https://docs.rs/spectacular).
#[proc_macro_attribute]
